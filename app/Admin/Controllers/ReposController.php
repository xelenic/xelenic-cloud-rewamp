...
//        dd();
        ...
        if($githubToken = Admin::user()->github_token){
            $grid = new Grid(new Repos());
            self::feedRepoData($githubToken);
...
">'.$name.'</a>';
            ...
            $grid->column('full_name', __('Full name'))->display(function ($full_name) {
                $str = '<span class="badge bg-primary" style="background: #5f7bce !important">' .$full_name.'</span> ';
                return $str;
            });
            $grid->language()->display(function ($language) {
                $str = '<span class="badge rounded-pill bg-success" style="">'.$language.'</span> ';
                return $str;
            });
            $grid->column('stargazers_count', __('Stargazers count'))->display(function ($stargazers_count) {
                $str = '<span class="badge rounded-pill bg-warning" style="background: #3383df !important"><i class="icon-star"></i> '.$stargazers_count.'</span> ';
                return $str;
            });
...
//            $actions->row('<a href="" class="btn btn-sm btn-info">Details</a>');
           ...
           return \view('backend.components.repo_authentication');
        }
